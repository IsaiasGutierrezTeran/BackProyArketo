...
def risk_report_pdf(analysis) -> bytes:
    ...
    doc = SimpleDocTemplate(
        buf,
        pagesize=A4,
        title=f"Análisis de riesgos #{analysis.id}",
        leftMargin=18 * mm,
        rightMargin=18 * mm,
        topMargin=18 * mm,
        bottomMargin=18 * mm,
    ...
    story = [
        Paragraph(
            "Arketo — Análisis de riesgos estructurales", styles["Title"]
        ),
        Paragraph(
    ...
    if not findings:
        story.append(
            Paragraph(
                "No se detectaron riesgos en este modelo.", styles["Normal"]
            )
        )
    else:
        rows = [
            ["#", "Severidad", "Categoría", "Descripción", "Recomendación"]
        ]
        for i, f in enumerate(findings, start=1):
            rows.append(
                [
                    str(i),
                    _SEVERITY_ES.get(f.severity, f.severity),
                    f.category,
        ...
        table = Table(
            rows,
            colWidths=[8 * mm, 20 * mm, 30 * mm, 60 * mm, 56 * mm],
            repeatRows=1,
        )
        style = TableStyle(
            [
        ...
        story += [
            Paragraph(
                f"Riesgos detectados: {len(findings)}", styles["Heading2"]
            ),
            table,
        ]
...
def budget_report_pdf(budget) -> bytes:
    ...
    for it in budget.items.select_related("material").all():
        rows.append(
            [
                ...
                f"{it.quantity}",
                f"{it.unit_price_snapshot} {cur}",
                f"{it.subtotal} {cur}",
            ]
        )
    table = Table(
        rows, colWidths=[78 * mm, 28 * mm, 33 * mm, 35 * mm], repeatRows=1
    )
    table.setStyle(
        TableStyle(
            [
    ...
    summary = [
        ...
        [
            f"Mano de obra ({budget.labor_people} personas)",
            f"{budget.labor_cost} {cur}",
        ],
    ...
    if review is not None:
        story += [
            Spacer(1, 6 * mm),
            Paragraph("Revisión del ingeniero", styles["Heading2"]),
            Paragraph(
                f"Decisión: {review.decision} · Revisor: "
                f"{getattr(review.reviewer, 'email', 'n/d')}",
                styles["Normal"],
            ),
        ]
        if review.comments:
            story.append(
                Paragraph(
                    f"Observaciones: {review.comments}", styles["Normal"]
                )
            )
