...
class ProjectViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if (
            limit is not None
            and Project.objects.filter(owner=self.request.user).count()
            >= limit
        ):
            raise ApiException(
    ...
    @extend_schema(
        responses=DashboardSerializer, summary="Resumen del dashboard (CU16)"
    )
    ...
    def sync(self, request):
        ...
        return Response(
            SyncSerializer(data, context={"request": request}).data
        )

    @extend_schema(
        summary="Listar colaboradores / invitar (elige usuario por id) (CU14)"
    )
    ...
    def members(self, request, pk=None):
        ...
        if request.method == "POST":
            ...
            membership = services.invite_member(
                ...
                user_id=data.validated_data["user"],
    ...
    @extend_schema(
        responses={200: AssignableUserSerializer(many=True)},
...
class CommentViewSet(
    ...
    def perform_create(self, serializer):
        ...
        if (
            not services.projects_for(self.request.user)
...
class InvitationViewSet(mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    def accept(self, request, pk=None):
        membership = services.accept_invitation(
            user=request.user, membership_id=pk
        )
        return Response(
            InvitationSerializer(membership, context={"request": request}).data
        )
