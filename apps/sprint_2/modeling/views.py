...
class Model3DViewSet(
    ...
    def scene(self, request, pk=None):
        ...
        model = services.replace_scene(model=model, scene_json=data.validated_data["scene"])
        return Response(Model3DSerializer(model, context={"request": request}).data)

    @extend_schema(summary="Exportar el modelo (URL del GLB)", responses={200: None})
    ...
    def import_model(self, request):
        ...
        data.is_valid(raise_exception=True)
        project = projects_for(request.user).filter(pk=data.validated_data["project"]).first()
        if project is None:
            raise ApiException("Proyecto no encontrado.", code="not_found", status_code=404)
        assert_can_edit_project(request.user, project)
        model = services.import_glb(project=project, glb_file=data.validated_data["file"])
        return Response(
