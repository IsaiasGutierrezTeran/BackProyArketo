...
def _car(d: ImageDraw.ImageDraw, box) -> None:
    x1, y1, x2, y2 = box
    d.rounded_rectangle([x1, y1, x2, y2], radius=16, fill=CAR, outline=(120, 126, 138), width=2)
    h = y2 - y1
    d.rounded_rectangle([x1 + 8, y1 + h * 0.18, x2 - 8, y1 + h * 0.42], radius=8, fill=CAR_GLASS)
    d.rounded_rectangle([x1 + 8, y1 + h * 0.56, x2 - 8, y1 + h * 0.80], radius=8, fill=CAR_GLASS)
...
def render_plan(rooms, w: int = 900, h: int = 680) -> bytes:
    ...
    for (x1, y1, x2, y2, label, cars) in rooms:
        ...
        d.text(((x1 + x2) / 2 - (tb[2] - tb[0]) / 2, (y1 + y2) / 2 - (tb[3] - tb[1]) / 2),
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        owner = (User.objects.filter(email="cliente@arketo.dev").first()
                 or User.objects.filter(role=Role.CLIENTE).first()
                 or User.objects.first())
        if owner is None:
            self.stdout.write(self.style.WARNING("No hay usuarios; corre seed_users primero."))
            return

        project, _ = Project.objects.get_or_create(
            owner=owner, name="Planos de ejemplo",
            defaults={"status": ProjectStatus.ACTIVE,
        ...
        if project.plans.count() >= expected:
            self.stdout.write(self.style.SUCCESS("seed_example_plans: ya estaba sembrado."))
            return

        created = 0
        if sources:
            for path in sources:
                self._add_plan(project, owner, path.read_bytes(), path.suffix.lstrip(".").lower(), path.name)
                created += 1
                self.stdout.write(self.style.SUCCESS(f"  + (real) {path.name}"))
        else:
    ...
    def _add_plan(self, project, owner, data: bytes, ext: str, label: str) -> None:
        fmt = PlanFormat.JPG if ext in {"jpg", "jpeg"} else PlanFormat.PNG
        plan = Plan.objects.create(
            project=project, uploaded_by=owner, original_format=fmt, size_bytes=len(data),
        )
        safe = "".join(c if c.isalnum() else "_" for c in label)[:40]
        plan.file.save(f"example_{project.id}_{safe}.{ 'jpg' if fmt == PlanFormat.JPG else 'png' }",
