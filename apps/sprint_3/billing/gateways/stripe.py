...
class StripeGateway(BillingGatewayBase):
    ...
    def _key(self) -> str:
        if not settings.STRIPE_SECRET_KEY:
            raise ApiException(
                "STRIPE_SECRET_KEY no configurada.", code="bad_request", status_code=400
            )
    ...
    def start_subscription(self, *, user, plan) -> GatewayResult:
        if not plan.stripe_price_id:
            raise ApiException(
                f"El plan '{plan.code}' no tiene stripe_price_id.", code="bad_request"
            )
        data = {
            "mode": "subscription",
        ...
        try:
            resp = requests.post(_CHECKOUT_URL, data=data, auth=(self._key(), ""), timeout=30)
        except requests.RequestException as exc:
            raise ApiException("No se pudo contactar Stripe.", code="inference_error",
                               status_code=502) from exc
        if resp.status_code >= 400:
            raise ApiException(f"Stripe respondió {resp.status_code}.",
