...
_SIZE = re.compile(r"(\d+(?:[.,]\d+)?)\s*(?:x|por|×)\s*(\d+(?:[.,]\d+)?)", re.IGNORECASE)
WALL = (28, 32, 40)
ROOM = (226, 238, 248)
TEXT = (44, 52, 66)
...
class MockSketchProvider(SketchProviderBase):
    ...
    def generate(self, prompt: str) -> bytes:
        ...
        d.text((40, 60), (prompt or "").strip()[:80], fill=(120, 130, 145), font=_font(16))
        ...
        d.rectangle([(x0 + midx) // 2 - 30, y1 - 4, (x0 + midx) // 2 + 30, y1 + 4], fill="white")

        f = _font(20)
        d.text(((x0 + midx) // 2 - 30, (y0 + midy) // 2), "Hab. 1", fill=TEXT, font=f)
        d.text(((x0 + midx) // 2 - 30, (midy + y1) // 2), "Hab. 2", fill=TEXT, font=f)
        d.text(((midx + x1) // 2 - 30, midy - 10), "Estar", fill=TEXT, font=f)
        d.text((x0 + 10, y1 + 12), f"~ {w:g} x {h:g} m", fill=(120, 130, 145), font=_font(16))
