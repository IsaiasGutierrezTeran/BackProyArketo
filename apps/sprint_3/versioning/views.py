...
class ProjectVersionViewSet(
    ...
    @extend_schema(request=CommitSerializer, responses={201: ProjectVersionSerializer},
    ...
    def commit(self, request):
        ...
        return Response(
            ProjectVersionSerializer(version, context={"request": request}).data,
            status=status.HTTP_201_CREATED,
        )

    @extend_schema(request=None, responses={200: ProjectVersionSerializer},
                   summary="Restaurar el proyecto a esta versión")
    @action(detail=True, methods=["post"])
    def restore(self, request, pk=None):
        version = services.restore_version(user=request.user, version=self.get_object())
        return Response(ProjectVersionSerializer(version, context={"request": request}).data)

    @extend_schema(
        parameters=[
            OpenApiParameter("from", int, required=True, description="Id de la versión base"),
            OpenApiParameter("to", int, required=True, description="Id de la versión destino"),
        ],
        responses={200: dict},
    ...
    def diff(self, request):
        ...
        if not from_id or not to_id:
            raise ApiException(
                "Indica 'from' y 'to' (ids de versión).", code="bad_request", status_code=400
            )
        base = qs.filter(pk=from_id).first()
        target = qs.filter(pk=to_id).first()
        if base is None or target is None:
            raise ApiException("Versión no encontrada.", code="not_found", status_code=404)
        if base.project_id != target.project_id:
            raise ApiException(
                "Las versiones son de proyectos distintos.", code="bad_request", status_code=400
            )
