...
class DesignRequest(BaseModel):
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="design_requests"
    )
    project = models.ForeignKey(
        "projects.Project", on_delete=models.CASCADE,
        null=True, blank=True, related_name="design_requests",
    )
    mode = models.CharField(max_length=12, choices=DesignMode.choices)
    prompt_text = models.TextField(blank=True)
    audio_file = models.FileField(upload_to="ai_design/audio/", null=True, blank=True)
    transcript = models.TextField(blank=True)
    provider = models.CharField(max_length=20, blank=True)
    status = models.CharField(
        max_length=12, choices=DesignStatus.choices, default=DesignStatus.PENDING
    )
    ...
    model3d = models.ForeignKey(
        ...
        null=True, blank=True, related_name="design_request",
    )
