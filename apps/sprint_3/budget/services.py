...
_R_FIERRO_PISO = Decimal("0.7")       # varillas 1/2" por m² (estimación estructural gruesa)
_R_CERAMICA = Decimal("1.05")         # m² de cerámica por m² de piso (5% desperdicio)
_R_PINTURA_GAL = Decimal("0.05")      # galón por m² de muro (~0.2 L/m² ÷ 3.785 L/galón)
...
def _find_material(*needles, unit: str | None = None) -> Material | None:
    ...
    for needle in needles:
        mat = base.filter(name__icontains=needle).order_by("unit_price").first()
        if mat is not None:
            return mat
    return None
...
def estimate_items_from_model(model3d) -> tuple[list[dict], dict]:
    ...
    for w in walls:
        ...
        pts += [(sx, sy), (ex, ey)]
        wall_area += math.hypot(ex - sx, ey - sy) * float(w.get("height") or 2.7)
    ...
    plan = [
        (_find_material("Ladrillo 6 huecos", "Ladrillo gambote", "Ladrillo"),
         Decimal(str(wall_area * _R_LADRILLO_M2))),
        (_find_material("Cemento Portland IP-30", "Cemento Portland", "Cemento"),
        ...
        (_find_material("Fierro corrugado 1/2", "Fierro corrugado 3/8", "Fierro"),
         fa * _R_FIERRO_PISO),
        (_find_material("Cerámica", "Piso cerámico", "Porcelanato"), fa * _R_CERAMICA),
...
def estimate_budget(*, user, model3d_id: int, currency: str | None = None) -> Budget:
    """Crea un presupuesto BORRADOR estimado desde la geometría del modelo 3D."""
    from modeling.models import Model3D  # import perezoso (evita ciclos de carga)

    model = Model3D.objects.filter(pk=model3d_id, project__in=projects_for(user)).first()
    if model is None:
    ...
    return create_budget(
        user=user, project_id=model.project_id, items=items,
        labor_people=0, labor_cost=None, currency=currency,
    )
